...
@Component({
  selector: 'app-domaine-pv',
  templateUrl: './domaine-pv.component.html',
  styleUrls: ['./domaine-pv.component.css']
})
export class DomainePvComponent implements OnInit {
...
years
  ngOnInit(): void {
    this.dashbordmasterService.get_classffiable().subscribe(data=>{
    ...
    chercher(){
      ...
      if(this.acceptanceselected==undefined&&this.classificationselected==undefined&&this.servicenameselected==undefined){
        if(this.Annee==undefined){
          this.dashbordmasterService.get_classffiable().subscribe(
            (data)=>{
              this.pieChartLabels = Object.getOwnPropertyNames(data);
              let values=Object.values(data)
                 this.pieChartData=values.map( s=>((Number(s)/1000)))
          ...
          this.dashbordmasterService.post_classdate(this.Annee).subscribe(data=>{
            this.pieChartLabels = Object.getOwnPropertyNames(data);
            let values=Object.values(data)
        ...
        else{ this.dashbordmasterService.post_dashP(this.acceptanceselected,this.Annee,this.classificationselected,this.servicenameselected).subscribe(data=>{
